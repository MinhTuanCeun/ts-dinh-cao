...
debugger;
		...
		getState: function(map,arr){
			...
			// Sort Data tr? v? theo T?nh thành và qu?n huy?n
		...
		getStoreDefault: function(arr){
			...
			$.each(arr,function(i,store){
				...
				if(store.state_name == 'H? Chí Minh'){
					...
					html += '<a class="view_popup_store" onclick="TCH.openModal(this)" href="javascript:void(0)" data-name="'+store.external_name+'" data-fulladdress="'+store.address.full_address+'" data-image1="'+img1+'" data-image2="'+ img2 +'" data-image3="'+img3+'" data-latlng="'+store.latitude+','+store.longitude+'">Chi ti?t c?a hàng</a></div></li>';
				}
			})
		...
		loopStore: function(arr,position_lat,position_lng){
			...
			if(arrLists.length != 0){
				...
				$.each(arrLists,function(i,store){
					...
					html += '<a class="view_popup_store" onclick="TCH.openModal(this)" href="javascript:void(0)" data-name="'+arr[store.index].external_name+'" data-fulladdress="'+arr[store.index].address.full_address+'" data-image1="'+img1 +'" data-image2="'+img2 +'" data-image3="'+img3 +'" data-latlng="'+arr[store.index].latitude+','+arr[store.index].longitude+'">Chi ti?t c?a hàng</a><span>'+ changeMetToKM +' </span></div></li>';
					if(i == 4) return false;
				})
				$('.list-address ul').html(html);
			}
			...
			$.each(arr,function(i,store){
				...
				if(position_lat == '' || position_lng == ''){
					...
					if(str == district_select){
						...
						html += '<img src="'+store.images[0].replace(/(^\w+:|^)/, '') +'" alt="'+store.external_name+'"/></div><div class="info">';
						html += '<h3>'+store.external_name+'</h3><p>'+store.street + ', ' + store.state_name +'</p>';
						html += '<a class="view_popup_store" onclick="TCH.openModal(this)" href="javascript:void(0)" data-name="'+store.external_name+'" data-fulladdress="'+store.address.full_address+'" data-image1="'+img1+'" data-image2="'+ img2 +'" data-image3="'+img3+'" data-latlng="'+store.latitude+','+store.longitude+'">Chi ti?t c?a hàng</a><span>'+ changeMetToKM +' </span></div></li>';
						count++;
					}
				}
			});
		...
		initMap: function(){
			...
			// Ð? d? li?u c?a t?nh thành
			that.getState(map,arrStore);

			// Thêm btn l?y v? trí hi?n t?i
			that.addYourLocationButton(map,marker,arrStore);

			// Ki?m tra và l?y v? tr? hi?n t?i
			that.getMyLocation(marker,function(position){
				var position_lat = '', position_lng = '';
		...
		slug: function(str) {
			...
			str = str.replace(/!|@|%|\^|\*|\(|\)|\+|\=|\<|\>|\?|\/|,|\.|\:|\;|\'| |\"|\&|\#|\[|\]|~|$|_/g, "-");
			str = str.replace(/-+-/g, "-"); //thay th? 2- thành 1- 
			str = str.replace(/^\-+|\-+$/g, "");
			return str;
		},
